...
@section('title', 'Dasbor Kepala Pusat')
...
@section('content')
    ...
    <div class="container">
            ...
            <div class="col-12">
                <div class="card text-white shadow-sm" style="background-color: #8BC3B4; border: none;">
                    ...
                    <div class="card-body p-4">
                        <h5 class="card-title fw-bold">Dasbor Kepala Pusat</h5>
                        <p class="card-text mb-0">Selamat datang. Kelola validasi proposal skema pusat yang masuk.</p>
                    </div>

                    {{-- Bagian Kartu Validasi (Rata Tengah) --}}
                    <div class="p-4 pt-0">
                        <div class="row justify-content-center g-3">
                            <div class="col-lg-4 col-md-5 col-sm-6">
                                <a href="{{ route('kepala_pusat.validasi') }}" class="text-decoration-none action-card-link">
                                    <div class="card shadow-sm action-card h-100" style="border-radius: 0.5rem; border: none;">
                                        <div class="card-body text-center p-4 position-relative">
                                            ...
                                            <h6 class="card-title mb-0 mt-2 action-title" style="color: #378a75;">Validasi Proposal Pusat</h6>
        ...
        {{-- Menunggu Persetujuan (STYLE WAKIL DEKAN 3) --}}
        <div class="row mb-4">
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-white border-bottom-0 pt-3 pb-2">
                        <h6 class="fw-bold mb-0 text-secondary text-uppercase" style="letter-spacing: 0.5px;">
                            <i class="bi bi-clock-history me-2"></i>Menunggu Persetujuan Anda
                        </h6>
                    </div>
                            ...
                            <div class="list-group list-group-flush">
                                <div class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                                    <div>
                                        ...
                                        {{-- META INFO (Tanggal • Pengusul • Skema • Status) --}}
                                        <div class="d-flex align-items-center flex-wrap gap-2">
                                            {{-- 1. Tanggal Masuk --}}
                                            <span class="text-secondary" style="font-size: 1rem;">
                                                Tanggal Masuk: <span class="fw-bold text-dark">{{ $lastPending->created_at->format('d-m-Y') }}</span>
                                            </span>

                                            <span class="text-secondary mx-1 d-none d-md-inline">•</span>

                                            {{-- 3. Skema (Accessor Model) --}}
                                            <span class="text-secondary" style="font-size: 1rem;">
                                                Skema: <span class="fw-bold text-dark">{{ $lastPending->skemaRef->nama ?? '-' }}</span>
                                            </span>

                                            <span class="text-secondary mx-1 d-none d-md-inline">•</span>

                                            {{-- 4. Status (Accessor Model) --}}
                                            <span class="text-secondary" style="font-size: 1rem;">
                                                Status:
                                                <span class="badge bg-{{ $lastPending->status_color }} bg-opacity-10 text-{{ $lastPending->status_color }} fs-6 px-3 py-1 ms-1">
                                    ...
                                    {{-- Tombol Aksi --}}
                                    <a href="{{ route('kepala_pusat.validasi.detail', $lastPending->id) }}" class="btn btn-outline-primary fw-bold px-4 py-2 ms-3 shadow-sm">
                                        Lihat Detail
                                    </a>
                                </div>
                            </div>
        ...
        <div class="row mb-4">
            <div class="col-12">
                <h6 class="text-uppercase fw-bold text-secondary mb-3">Informasi Pusat</h6>
                <div class="row g-3">
                    {{-- Identitas --}}
                    <div class="col-lg-3 col-md-12">
                        <div class="card shadow-sm h-100 border-0">
                            <div class="card-body p-3 d-flex flex-column justify-content-center align-items-center text-center">
                                ...
                                <span class="badge bg-light text-dark border px-2 py-1 mt-1 mb-2" style="font-size: 0.75rem;">NIDN: {{ auth()->user()->nidn ?? '-' }}</span>
                                <div class="mt-2 text-muted small">Kepala Pusat</div>
                            </div>
                        </div>
                    </div>
                    
                    {{-- Load Partial Statistik --}}
                    @include('kepala_pusat.partials.dashboard_stats_cards')
                </div>
            </div>
        </div>
    </div>
