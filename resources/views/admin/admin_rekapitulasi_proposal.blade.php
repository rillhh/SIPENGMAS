...
@section('content')
    <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
            ...
            {{-- SHARED TOOLBAR (SEARCH + YEAR + EXCEL) --}}
            <div class="d-flex gap-2">
                {{-- Tombol Excel (Pastikan route export_excel sudah ada di web.php) --}}
        ...
        <ul class="nav nav-tabs" id="rekapTab" role="tablist">
            <li class="nav-item">
                <button class="nav-link active" id="proses-tab" data-bs-toggle="tab" data-bs-target="#proses-pane" type="button" onclick="setActiveTab('proses')">
                    Proses <span class="badge bg-primary text-white ms-1">{{ $data['proses']->total() }}</span>
                </button>
            </li>
            <li class="nav-item">
                <button class="nav-link" id="didanai-tab" data-bs-toggle="tab" data-bs-target="#didanai-pane" type="button" onclick="setActiveTab('didanai')">
            ...
            <div class="card-body p-0">
                <div class="tab-content" id="rekapTabContent">
                    ...
                    <div class="tab-pane fade show active" id="proses-pane">
                        @include('admin.partials.table_rekap', ['items' => $data['proses'], 'type' => 'proses'])
                    </div>
                    
                    {{-- TAB: DIDANAI --}}
                    <div class="tab-pane fade" id="didanai-pane">
                        @include('admin.partials.table_rekap', ['items' => $data['didanai'], 'type' => 'didanai'])
                    </div>
                    
                    {{-- TAB: DITOLAK --}}
                    <div class="tab-pane fade" id="ditolak-pane">
                        @include('admin.partials.table_rekap', ['items' => $data['ditolak'], 'type' => 'ditolak'])
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function setActiveTab(tabName) {
        ...
        // Update link excel agar sesuai tab aktif
...
@push('styles')
    <style>
        #rekapTab .nav-item .nav-link { color: #555 !important; background-color: #f8f9fa; border-bottom: none; margin-right: 2px; }
        #rekapTab .nav-item .nav-link.active { color: #378a75 !important; background-color: #fff; border-color: #dee2e6 #dee2e6 #fff; font-weight: 600; }
        .text-primary { color: #378a75 !important; }
        .btn-success { background-color: #378a75; border-color: #378a75; }
        .btn-success:hover { background-color: #2e7361; border-color: #2e7361; }
    </style>
@endpush