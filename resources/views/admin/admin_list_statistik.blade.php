        ...
        <div class="d-flex gap-2">
            <form action="{{ url()->current() }}" method="GET" class="d-flex" style="max-width: 300px;">
                ...
                <div class="input-group shadow-sm">
                    ...
                    <input type="text" class="form-control border-start-0 ps-0" name="search" placeholder="Cari judul/nama..." value="{{ $search ?? '' }}">
                    <button class="btn btn-success text-white fw-bold" type="submit">Cari</button>
                </div>
            </form>
            <a href="{{ route('admin.dashboard') }}" class="btn btn-secondary shadow-sm d-flex align-items-center fw-bold px-3">
                        ...
                        <tr>
                            ...
                            @else
                                ...
                                <th class="py-3 text-secondary" style="width: 25%;">
                                    @if($kategori == 'sertifikat') Judul Sertifikat
                                    @elseif($kategori == 'hki') Judul HKI
                                    @else Judul Artikel
                                    @endif
                                </th>
                                <th class="text-center py-3 text-secondary">Aksi</th>
                        ...
                        @forelse($items as $index => $item)
                            <tr class="border-bottom">
                                ...
                                @if(in_array($kategori, ['proposal_keseluruhan', 'proposal_didanai']))
                                    <td class="py-3">
                                        <div class="d-flex flex-column">
                                            {{-- Akses via relasi: $item->identitas->judul --}}
                                    ...
                                    {{-- Kolom Total Dana (Gunakan Accessor) --}}
                                    <td class="text-end py-3 pe-4">
                                        <span class="fw-bold text-success" style="font-family: monospace; font-size: 0.95rem;">
                                            Rp {{ number_format($item->total_dana, 0, ',', '.') }}
                                        </span>
                                    </td>

                                    {{-- Kolom Status (Gunakan Accessor) --}}
                                    <td class="text-center py-3">
                                        <span class="badge bg-{{ $item->status_color }} bg-opacity-10 text-{{ $item->status_color }} rounded-pill px-3 border border-{{ $item->status_color }} border-opacity-25">
                                            {{ $item->status_label }}
                                        </span>
                                    </td>
                                ...
                                @else
                                    ...
                                    {{-- Skema dari Proposal Induk (Gunakan Accessor pada relasi proposal) --}}
                                    <td class="py-3 text-dark small fw-bold">
                                    ...
                                    <td class="text-center py-3">
                                        <a href="{{ isset($item->file_path) ? \Storage::url($item->file_path) : '#' }}" target="_blank" class="btn btn-sm btn-outline-success fw-bold rounded-pill px-3">
                                            <i class="bi bi-download me-1"></i> Unduh
                                        </a>
                                    </td>

                                    <td class="text-center py-3">
                                        <a href="{{ route('admin.rekapitulasi.detail', $item->proposal_id) }}" class="btn btn-sm btn-primary rounded-pill px-3 shadow-sm text-white" style="background-color: #378a75; border-color: #378a75;">
                                            Detail
                                        </a>
                                    </td>
                                @endif
