...
@section('content')
    <div class="container">
        {{-- 1. LOGIKA BACK BUTTON BERDASARKAN ROLE --}}
        @php
            $role = auth()->user()->role;
            // Tentukan route kembali berdasarkan role
            $backRoute = match (true) {
                str_contains($role, 'Admin') => 'admin.dashboard',
                str_contains($role, 'Dosen') => 'dosen.dashboard',
                str_contains($role, 'Wakil Dekan') => 'wakil_dekan3.dashboard', // Sesuaikan
                str_contains($role, 'Dekan') => 'dekan.dashboard',
                str_contains($role, 'Kepala Pusat') => 'kepala_pusat.dashboard', // Sesuaikan
                str_contains($role, 'Wakil Rektor') => 'wakil_rektor.dashboard', // Sesuaikan
                default => 'login',
            };
        @endphp
                ...
                <form id="form-update-profile" method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 border-end pe-md-4">
                            ...
                            {{-- B. INPUT TANDA TANGAN (CUSTOM LOGIC) --}}
                            @php
                                // Daftar Role yang Diizinkan (Sesuai Permintaan)
                                $allowedKeywords = [
                                    'Dosen', 
                                    'Dekan', 
                                    'Kepala Pusat 1',
                                ...
                                // Logika 1: Khusus Dosen (Pakai Contains agar 'Dosen Pengusul' masuk)
                                if (\Illuminate\Support\Str::contains($user->role, 'Dosen')) {
                                    $canUploadSignature = true;
                                } 
                                // Logika 2: Role Lainnya (Pakai in_array / Exact Match)
                                // Ini PENTING agar 'Wakil Dekan' TIDAK dianggap 'Dekan'
                                    ...
                                    <div class="card bg-light border-0 p-3">
                                        ...
                                        @if($user->tanda_tangan)
                                            <div class="mb-3 text-center bg-white border rounded p-2" style="border-style: dashed !important;">
                                                ...
                                                <img src="{{ Storage::url($user->tanda_tangan) }}" alt="Tanda Tangan" class="img-fluid" style="max-height: 100px; object-fit: contain;">
                                            </div>
                                        @endif

                                        {{-- Input File --}}
                                        <input type="file" class="form-control @error('tanda_tangan') is-invalid @enderror" 
                        ...
                        <div class="col-md-6 ps-md-4 mt-4 mt-md-0">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('fakultas') is-invalid @enderror" id="fakultas_edit" name="fakultas" onchange="updateProdiEdit()" required>
                                    <option value="" disabled selected>Pilih Fakultas</option>
                                    
                                    {{-- Loop Data dari Database --}}
                                    @foreach($fakultas as $f)
                                        <option value="{{ $f->nama }}" {{ old('fakultas', $user->fakultas) == $f->nama ? 'selected' : '' }}>
                                            {{ $f->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('prodi') is-invalid @enderror" id="prodi_edit" name="prodi" required>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('jabatan_fungsional') is-invalid @enderror" id="jabatan_fungsional" name="jabatan_fungsional" required>
                                    <option value="" disabled {{ !$user->jabatan_fungsional ? 'selected' : '' }}>Pilih Jabatan</option>
                                    
                                    {{-- Loop Data dari Database --}}
                                    @foreach($jabatans as $j)
                                        <option value="{{ $j->nama }}" {{ old('jabatan_fungsional', $user->jabatan_fungsional) == $j->nama ? 'selected' : '' }}>
                                            {{ $j->nama }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                        <a href="https://www.yarsi.ac.id/ganti-password-akun-yarsi" target="_blank" class="btn btn-outline-primary fw-bold px-4">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                ...
                <div class="modal-footer justify-content-center bg-light border-0">
                    ...
                    <button type="submit" form="form-update-profile" class="btn btn-primary fw-bold px-4">Ya, Simpan</button>
                </div>
            </div>
        </div>
    </div>

    {{-- 6. SCRIPT DATA PRODI DINAMIS --}}
    <script>
        ...
        function updateProdiEdit() {
            ...
            // Ambil data prodi user saat ini dari Blade agar bisa di-autoselect saat load
            ...
            if (selectedFakultas && prodiDataEdit[selectedFakultas]) {
                prodiDataEdit[selectedFakultas].forEach(prodi => {
                    ...
                    // Auto-select jika cocok dengan data user saat ini
