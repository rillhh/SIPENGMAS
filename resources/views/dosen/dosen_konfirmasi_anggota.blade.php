...
@section('content')
    <div class="container">
        ...
        <ul class="nav nav-tabs" id="anggotaTab" role="tablist">
            ...
            <li class="nav-item">
                <button class="nav-link" id="tunggu-tab" data-bs-toggle="tab" data-bs-target="#tunggu-pane" type="button" onclick="setActiveTab('tunggu')">
                    Menunggu Tim <span class="badge bg-warning text-dark ms-1">{{ $data['waiting']->total() }}</span>
                </button>
            </li>
            <li class="nav-item">
                <button class="nav-link" id="proses-tab" data-bs-toggle="tab" data-bs-target="#proses-pane" type="button" onclick="setActiveTab('proses')">
                    Proses Review <span class="badge bg-info text-dark ms-1">{{ $data['process']->total() }}</span>
                </button>
            </li>
            <li class="nav-item">
                <button class="nav-link" id="selesai-tab" data-bs-toggle="tab" data-bs-target="#selesai-pane" type="button" onclick="setActiveTab('selesai')">
            ...
            <div class="card-body p-0">
                <div class="tab-content" id="anggotaTabContent">
                    ...
                    <div class="tab-pane fade show active" id="menunggu-pane">
                        ...
                        @include('dosen.partials.table_anggota', ['items' => $data['invitations'], 'type' => 'invitation'])
                    </div>

                    {{-- TAB 2: MENUNGGU TIM --}}
                    <div class="tab-pane fade" id="tunggu-pane">
                        @include('dosen.partials.table_anggota', ['items' => $data['waiting'], 'type' => 'waiting'])
                    </div>

                    {{-- TAB 3: PROSES REVIEW --}}
                    <div class="tab-pane fade" id="proses-pane">
                        @include('dosen.partials.table_anggota', ['items' => $data['process'], 'type' => 'process'])
                    </div>

                    {{-- TAB 4: SELESAI --}}
                    <div class="tab-pane fade" id="selesai-pane">
                        @include('dosen.partials.table_anggota', ['items' => $data['finished'], 'type' => 'finished'])
                    </div>
...
@push('styles')
    <style>
        #anggotaTab .nav-item .nav-link { color: #555 !important; background-color: #f8f9fa; border-bottom: none; margin-right: 2px; }
        #anggotaTab .nav-item .nav-link.active { color: #378a75 !important; background-color: #fff; border-color: #dee2e6 #dee2e6 #fff; font-weight: 600; }
        .status-menunggu { color: #664d03; background-color: #fff3cd; border: 1px solid #ffc107; padding: 0.5em 0.75em; font-size: 0.75rem; }
        .status-selesai { color: #0f5132; background-color: #d1e7dd; border: 1px solid #198754; padding: 0.5em 0.75em; font-size: 0.75rem; }
    </style>
@endpush